...
type BoardProps = {
    id: string  // Keep as string (URL params are strings)
...
const BoardView = ({id, setIsModalNewTaskOpen}: BoardProps) => {
  ...
  // Early return if invalid ID - this prevents the NaN error
  if (isNaN(projectId) || !id) {
    return (
      <div className="flex items-center justify-center p-8">
        <div className="text-red-600 dark:text-red-400">
          Invalid project ID: "{id}". Please check the URL.
        </div>
      </div>
    )
  }

  // Now projectId is guaranteed to be a valid number
  ...
  if (error) {
    ...
    return (
      <div className="flex items-center justify-center p-8">
        <div className="text-red-600 dark:text-red-400">
          Error while fetching tasks for project {projectId}
        </div>
      </div>
    )
  } 
...
const Task = ({task}: TaskProps) => {
  ...
  const formattedEndtDate = task.dueDate? format(new Date(task.dueDate),"P") : ""
  ...
  return (
    <div ref={(instance)=>{
      ...
      {task.attachments && task.attachments.length > 0 && (
        <Image src={`https://pm-s3-img-bucket.s3.ap-south-1.amazonaws.com/${task.attachments[0].fileURL}`} 
        alt={task.attachments[0].fileName} width={400} height={200} className='h-auto w-full rounded-t-md'/>
      )}
      <div className='p-4 md:p-6'>
        <div className='flex items-start justify-between'>
          ...
          <button className='flex h-6 wi-4 shrink-0 items-center justify-center dark:text-neutral-500'>
            <EllipsisVertical size={26}/>
          </button>
        </div>
        ...
        <div className='text-xs text-gray-500 dark:text-neutral-500'>
          ...
          {formattedEndtDate && <span>{formattedEndtDate}</span>}
        </div>
        <p className='text-xs text-gray-600 dark:text-neutral-500 font-semibold'>{task.description}</p>
          ...
          <div className='mt-3 flex items-center justify-between '>
            <div className='flex -space-x-[6px] overflow-hidden'>
              {task.assignee && (
                ...
                src={`https://pm-s3-img-bucket.s3.ap-south-1.amazonaws.com/${task.assignee.profilePictureUrl!}`} 
              ...
              {task.author && (
                ...
                src={`https://pm-s3-img-bucket.s3.ap-south-1.amazonaws.com/${task.author.profilePictureUrl!}`} 
